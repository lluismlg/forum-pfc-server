...
app.get("/api/searchTitle", (req, res) => {
	...
	con.query(`SELECT postId, DATE_FORMAT(postDate, "%d-%m-%Y") as postDate,postType, postAuthor, postLikes, postTitle, postImage, postContent, postTopic FROM posts WHERE postTitle LIKE "${searchTitle}"`, (err, results) => {
		if (err) {
			return res.send(err);
		} else {
			return res.send(results);
